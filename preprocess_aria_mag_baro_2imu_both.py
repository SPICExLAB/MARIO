...
class CPFWorldTransformer:
    ...
    def __init__(self, sequence_path: Path, skip_seconds: float = 30.0):
        ...
        self.vrs_path = sequence_path / f"AriaEverydayActivities_1.0.0_{sequence_name}_main_recording.vrs"
        self.traj_path = sequence_path / "closed_loop_trajectory.csv"
        self.calib_path = sequence_path / "online_calibration.jsonl"
    ...
    def _load_and_process_imu(self):
        ...
        for i in tqdm(range(num_samples_left), desc="IMU-left", leave=False):
            ...
            if self.imu_calib_left:
                ...
                # print("")
                # print(f"     Applied LEFT IMU calibration")
                # print(f"       Gyro bias: {self.imu_calib_left['gyro_bias']}")
                # print(f"       Accel bias: {self.imu_calib_left['accel_bias']}")
                # print(f"       R_Device_Imu:\n{self.imu_calib_left['R_Device_Imu']}")
                # print(f"       R_device_to_CPF_vectors:\n{self.R_device_to_CPF_vectors}")
                # print("")
            else:
                accel_device = accel_raw
                gyro_device = gyro_raw
            ...
            print("accel_left cpf:", accel_cpf)
...
# Example usage:
# python preprocess_aria_mag_baro_2imu_both.py $DATA_ROOT/aria_everyday_full -o $DATA_ROOT/aria_processed_cpf_both_testing --sample-hz 5 --parallel 1 --max 1 